...
impl ArmResult {
    ...
    /// baked: `run_producer_subsistence` (`phases.rs:971`) and
    /// `deliver_demography_provisions` (`demography.rs:1101`) both mint `known.hunger`,
    /// which on this chain IS bread (`generation.rs:84`), and both mints are live unless
    /// `retire_food_mints` is set — i.e. on `base` and `L2`.
    ///
