...
confound. The honest implication reorders the road ahead: because the loop will not *self-start*, the next lever is
not another voluntary-market institution (nor the firm/household/rent layers that all presuppose a living circular
flow) but an **outside bootstrap injector** — a state that hires and spends first (a fiscal/chartalist circuit,
building on the M21 result that a fiat tax already pulls idle labor into wage work), or a credit advance that fronts
wages against future output. C1 is preserved as a branch-only null (`feat/wage-labor-impl-rb`), not merged — its
spec merges only on `CircularFlowForms`, which it did not reach.
