...
## 32. The escape experiment runs — and the demand horizon closes it before the question can be asked (C3R.e)

The keystone's endgame put the diagnosis-selected lever to the strictest test the program has
preregistered: three finite interventions — a one-shot conserved redistribution of twenty-four loaves at
tick 50, a generation-time additive endowment of the same size, and the landed C3R.b staple support pair
withdrawn whole at tick 400 — on two disclosed regimes, against a six-criterion hysteresis bar with a
launder-proof intervention-origin flag (surviving even market resale), global exhaustion with
estate-to-commons as terminal disposal, drawn-lot birth-funding attribution, the landed structure and flow
bars with an all-goods revenue leg, live external demand per window, and a matched no-ignition twin
compared over identical absolute intervals. Only `HysteresisHolds` — five consecutive six-lifetime windows
with all criteria met — was permitted the language of multiple equilibria. The stated honest default was
relapse.

**Neither happened. The ladder returned undetermined-class verdicts across the grid, and that refusal is
the finding.** No relapse was proven, and no claim that the trap is unescapable is licensed. What the
thirty-five runs establish: the redistribution failed its pinned dose precondition on every seed — not one
loaf moved, and the result-review's deterministic re-derivation found the deeper reason: **every original
producer founder is dead by tick 36**, so the tick-50 intervention arrived at households of heirs or
extinct lines (the exact split among empty households, gate blocks, and donor scarcity was not
instrumented — disclosed). Most durable-regime results were voided by the preregistered guard because
**permanent support itself failed structure on four of five seeds** — the composite durable regime (the
producer warmth expense plus doubled gatherers) was markedly less robust than the landed economy, with the
producer expense the leading mechanism though not a proven sole cause. And in **every eligible measurement
window that any cell reached, on either regime, external demand was dead** (`ExternalDemandDiedFirst` at
window 560 in most cells, at 160 in one; criterion v is not degenerately wired — it passes in seed 19's
early observation window before demand dies). The two remaining current-regime seeds never formed
structure inside the early observation grid at all (`IgnitionNeverIgnites` — a stricter early-window lens
than the landed C3R.b final-window bar, and one of the two seeds never persisted in C3R.b either). The
matched references stayed trapped ten out of ten.

The keystone therefore closes not with an answer to the hysteresis question but with the measured
demonstration that **the question cannot yet be posed in this economy**: the demand base dies before any
escape can be evaluated, every path in — support, redistribution, endowment — is intercepted by the same
mortality-and-demand arithmetic that built the trap, and the regime engineered to supply durable demand
undermined the producers it was meant to serve. The program's oldest wall — no demand survives; the S23e
finding, the C1 finding, the C3R.c finding — is now also, precisely and demonstrably, the wall in front of
the trap's exit. Disclosures: the A1 gate decomposition is unresolved verification debt; A2's first-window
inventory (12 of 24) reflects post-tick-0 sampling of deliberately front-loaded lots, source-rederived; a
result-neutral eligibility-boundary mismatch (A2 `start > 0` vs the spec's `≥ 0`) must be fixed before the
oracle is reused. Spec impl-67, SPEC-READY after three xhigh rounds; rb-lite clean in one round;
result-review ACCEPT-AS-HONEST-FINDING with the corrected framing. Preserved on
`feat/ignition-withdrawal-impl-rb` (tip `f6d68a4`).
...
## 34. The reproductive-burden audit — the wall is the transfer itself, not the number four (DH.b)
...
row is therefore `CostlessOnlyReplacement`: replacement is mechanically possible exactly when
provisioning costs nothing, and every positive burden — down to a single loaf — is at rungs 2–3.
The `motive_effect` payload is empty in BOTH directions: the saving arm changed no cell's rung
anywhere on the grid, the same ~500:1 allocation loss C3R.d measured now visible as a flat
On-column. The `nonmonotone` payload is empty. The DH.c gate is NOT authorized — no q=4 arm at
rung 6+/7 on 5/5 seeds — so no grand-oracle rerun is licensed.

The answer to the preregistered question: the wall is robust, and it is sharper than the constant.
Producer-lineage extinction is not an artifact of the number four — the binding edge sits between
q=0 and q=1. ANY positive provisioning transfer keeps this closed colony from reproducing its
producer classes: at three-or-more loaves the gate never opens; at one or two the children arrive
into households that cannot hold a lineage through even one 180-tick streak; only literal
...
succession, and (on one seed) staffed flow are all attainable states of this base, so the q>0
verdicts are failures of the economy, not of the instrument. The reproduction frontier's next
slice inherits a tightened target: not "lower the burden" (no relaxation short of zero passes, and
the constant is imposed natural data) but the retained institutional option — pooled-heir
succession — now facing a wall known to bind at the very first loaf of provisioning.

Disclosures: the 60-cell suite costs ~105s wall (≈1.8s/run against the spec's ~1.6s estimate; the
...
| C3R.e | Ignition and withdrawal (civilization keystone, ENDGAME — can a finite intervention outlive its own removal?) | **FINDING — the escape experiment cannot yet be POSED: undetermined-class verdicts across the grid; the demand horizon binds before hysteresis is measurable (unmerged).** Three finite interventions (A1 one-shot conserved redistribution of 24 loaves @ tick 50, non-producer donors, dose detector; A2 generation-time additive endowment split into launder-proof intervention-origin lots at the ledger bootstrap; B the landed C3R.b staple support pair — food_provision=1 AND the cushion's staple leg — withdrawn whole @ 400, the cushion's untrackable WOOD leg disabled all run) × two regimes ({current}; {durable} = producer wood_provision=0 + gatherers=48), against the six-criterion bar: GLOBAL origin-flag exhaustion (the flag survives the market Bought-retag — order-preserving, adjacent-coalesce only; estate-to-commons = terminal disposal), drawn-lot birth funding (births ≥1, Bought|SelfProduced only), landed structure/flow + an all-goods revenue leg, live external demand (genuine trades AND gatherer purchases) per absolute 160-tick window, matched no-ignition twin on identical intervals; HysteresisHolds (any 5-window streak, checked FIRST) alone licensed multiple-equilibria language; relapse the stated default. Flag knobs under tag 33 (one fixed injective record). RESULT (35 runs, all guards hold, all landed suites reproduce): **A1 IgnitionShortfall ×5 — dose 0 everywhere; the RoR's deterministic re-derivation: every original producer founder is DEAD by tick 36 (lifespans 18–36), so the tick-50 shot arrived at heirs or extinct households** (exact gate split uninstrumented — disclosed debt). **Durable-regime cells voided ×4 by the preregistered guard: PERMANENT support fails structure on 4/5 seeds** — the composite regime (producer warmth expense + doubled gatherers) markedly less robust, the expense the leading mechanism, not a proven sole cause. **Every eligible window in every measuring cell: external demand dead** (EDD@560 mostly, @160 once; criterion v verified live — seed 19's early window passes v before demand dies). Current-regime seeds 7/19: IgnitionNeverIgnites under the stricter early-window observation lens. Matched references trapped 10/10. **Endorsed framing: no relapse proven, no unescapability claimed — the ladder refused every false claim; what is proven is that the demand base dies before any escape can be evaluated, and the program's oldest wall (S23e→C1→C3R.c: no demand survives) is now demonstrably the wall in front of the trap's exit.** Anomalies adjudicated: A2's first-window 12-of-24 = exact tick-0 consumption of front-loaded lots (6×2); one result-neutral eligibility-boundary mismatch (A2 start>0 vs spec ≥0) to fix before oracle reuse. Spec impl-67 SPEC-READY in 3 xhigh rounds (the resale-proof origin flag; era semantics; RegimeUntrapsWithoutIgnition; the B-WOOD closure); rb-lite clean in 1 round (claude); RoR ACCEPT-AS-HONEST-FINDING with corrected framing. Preserved on `feat/ignition-withdrawal-impl-rb` (tip `f6d68a4`). |
