...
Status (spec): **DRAFT — pre Codex spec-review.** Base: the **S23d branch** `feat/mortal-landowner-impl-rb`
...
**The mass-balance scarcity predeclaration (the anti-smuggling crux).** Let `c` = the per-agent per-tick
subsistence draw (a known base constant from the emergency floor), and `N` = the count of subsistence-seeking
(non-self-sufficient) agents on the base. The unlimited floor supplies `c·N`/tick (feeds all → S23d null). The
rival commons regenerates `r`/tick with buffer cap `K`; at steady state it sustains `r/c` foragers, so define
the **scarcity fraction** `φ = (r/c)/N`. **`φ` is the single predeclared axis, derived from mass balance, and
SWEPT — never searched:** `φ ∈ { abundant ≈ 1.25, marginal ≈ 0.5, scarce ≈ 0.25 }` (exact values pinned as
`const`s). Headline = **scarce** (`φ ≈ 0.25`: the commons can feed ~a quarter of seekers, the rest must trade or
starve). `K` = a small multiple of `r` (a few ticks of regen) so depletion bites but a short buffer exists.
`r = φ · c · N` is computed from the base, not chosen to pass.

**The rival draw (conservation-safe).** Each tick, hungry agents draw subsistence from the commons stock;
**total draw ≤ current stock** (rivalry); if demand exceeds stock, allocation is deterministic (pinned order,
e.g. by hunger then agent-id) and the unfed must **buy owner bread or go hungry**. The commons is a tracked good
with **regen as a named source** and **draw as a named sink** — the per-tick conservation identity holds exactly
(nothing minted beyond the declared regen).
...
2. **A real producer/buyer market forms** — non-owner buyers materially buy owner-produced bread
   (`final_buyer_bought ≥ MATERIAL_BOUGHT_FLOOR`), and the economy is **not subsistence-bound**
   (`final_commons_share ≤ MARKET_CAP`, i.e. the outside option is no longer the whole diet).
3. **Survival improves over the owner-surplus-disabled control** — final-window survival on the headline
   exceeds the `scarce + owner-surplus-disabled` control by ≥ `SURVIVAL_LIFT` (the market, not the scarcity
   alone, is what keeps people alive).
4. **Owners actually supply surplus under mortality** — instrumented: mean owner **surplus-sold-before-death**
   ≥ `OWNER_SURPLUS_FLOOR`, and buyer purchases are attributable to owner-produced (not seeded) bread
   (`seeded_minted == 0`). *(This is the clause that separates a real market from a fluke; see §4.)*
...
- `ScarcityStarves` — the commons is scarce but there is no owner supply to replace it, so agents die
  (`extinct` or survival below the S23d baseline): scarcity without supply is starvation, not a market. The
  `scarce + owner-surplus-disabled` control **must** land here (or non-viable) — proving you cannot fake a
  verdict by starving the outside option.
- `NoSurplusUnderMortality` — the commons is scarce AND owners *could* supply, but **short-lived owners do not
  accumulate and sell a marketable surplus before death** (instrumented: `owner_surplus_sold_before_death`
  below floor despite depleted commons and unmet demand). The deeper structural wall Codex flagged — the
  **"option C" dead-end signal** that another lever will not fix (needs lifespan / inherited-capital changes).
- `SubsistenceBoundDespiteScarcity` — scarce, owners supply *some* surplus, but the market still does not clear
  to viability (buyers don't materially buy / survival doesn't lift): a partial/negative result short of both
  success and the clean mortality wall.
...
`MoneyFailure` → `ScarcityStarves` (scarcity-caused mass death / survival below S23d baseline) →
`AbundanceReproducesNull` (abundant control only) → `NoSurplusUnderMortality` (scarce + demand unmet + owner
surplus-sold-before-death below floor) → `SubsistenceBoundDespiteScarcity` (scarce, some supply, market doesn't
clear) → **then the explicit final gate:** `if ALL SIX success clauses pass { ScarcityForcesMarket } else
{ report the first failed clause }`. Predeclare every threshold as a `const`; do not fit.
...
1. **A finite regenerating non-excludable commons pool.** A tracked subsistence stock with carrying cap `K` and
   regen `r`/tick (both derived from the mass-balance `φ` of §2). Non-excludable: any hungry agent may draw;
   rival: the sum of draws in a tick cannot exceed the current stock. Draw allocation on shortfall is
   deterministic (pinned order). Regen is a named conservation **source**, draw a named **sink**.
2. **The emergency subsistence path routes through the commons.** Where the S23d base minted unlimited own-labor
   emergency bread (`emergency_self_provision` / `emergency_bread_provisioned`), the flag re-routes the same
   hunger-relief step to **draw from the commons pool** instead — so when the pool is depleted, the step
   delivers less/none and the agent stays hungry (and must buy owner bread or die). No other S23d behavior
   changes.
...
**digest tag 21** (…, 18 = S23c, 19 = fixed_commitment_norm, 20 = S23d; 21 = this). Emit the commons state
(stock, `K`, `r`, `φ`) only when the flag is on; with it off, `canonical_bytes`/digest are byte-identical to the
S23d/base stream. The new mass-balance constants and instrumentation counters are test-visible telemetry, not
...
## 9. Codex spec-review resolutions

_(to be filled after Codex spec-review; iterate to SPEC-READY before rb-lite build.)_
